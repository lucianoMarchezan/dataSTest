...
public class ApoDefenceEnemyWerewolf extends ApoDefenceEnemy
{

	public ApoDefenceEnemyWerewolf( BufferedImage iEnemy, ArrayList<Point> enemyWay, int level )
	{
		super( ApoDefenceConstants.ENEMY_WEREWOLF, iEnemy, enemyWay );
		
		this.setValues( ApoDefenceConstants.ENEMY_WEREWOLF_NAME,
						level, 
						ApoDefenceConstants.ENEMY_WEREWOLF_HEALTH, 
						ApoDefenceConstants.ENEMY_WEREWOLF_HEALTH_UPGRADE,
						ApoDefenceConstants.ENEMY_WEREWOLF_SPEED, 
						ApoDefenceConstants.ENEMY_WEREWOLF_SPEED_UPGRADE, 
						ApoDefenceConstants.ENEMY_WEREWOLF_ARMOR, 
						ApoDefenceConstants.ENEMY_WEREWOLF_ARMOR_UPGRADE,
						ApoDefenceConstants.ENEMY_WEREWOLF_DAMAGE,
						ApoDefenceConstants.ENEMY_WEREWOLF_DAMAGE_UPGRADE,
						ApoDefenceConstants.ENEMY_WEREWOLF_MONEY * level / 2 );
		
		this.setBUndead( true );
	}
	
	public ApoDefenceEnemyWerewolf( BufferedImage iEnemy, ArrayList<Point> enemyWay, int level, int startX, int startY )
	{
		super( ApoDefenceConstants.ENEMY_WEREWOLF, iEnemy, enemyWay );
			...
			for ( int i = 0; i < tower.size(); i++ )
			{
				if ( tower.get( i ).getRec().intersects( this.getRangeRec() ) )
				{
					...
					if ( this.isIn( enemyX, enemyY, towerX, towerY ) )
					{
						...
						this.getShoot().add( new ApoDefenceShootEnemy( this, this.getX() - 3, this.getY() - 3, 7, 7, speedX, speedY, this.getDamage(), Color.GRAY ) );
						break;
					}
				}
			}
