...
public class ApoDefenceEnemySnake extends ApoDefenceEnemy
{

	public ApoDefenceEnemySnake( BufferedImage iEnemy, ArrayList<Point> enemyWay, int level )
	{
		super( ApoDefenceConstants.ENEMY_SNAKE, iEnemy, enemyWay );
		
		this.setValues( ApoDefenceConstants.ENEMY_SNAKE_NAME,
						level, 
						ApoDefenceConstants.ENEMY_SNAKE_HEALTH, 
						ApoDefenceConstants.ENEMY_SNAKE_HEALTH_UPGRADE,
						ApoDefenceConstants.ENEMY_SNAKE_SPEED, 
						ApoDefenceConstants.ENEMY_SNAKE_SPEED_UPGRADE, 
						ApoDefenceConstants.ENEMY_SNAKE_ARMOR, 
						ApoDefenceConstants.ENEMY_SNAKE_ARMOR_UPGRADE,
						ApoDefenceConstants.ENEMY_SNAKE_DAMAGE,
						ApoDefenceConstants.ENEMY_SNAKE_DAMAGE_UPGRADE,
						ApoDefenceConstants.ENEMY_SNAKE_MONEY * level / 2 );
	}
	
	public ApoDefenceEnemySnake( BufferedImage iEnemy, ArrayList<Point> enemyWay, int level, int startX, int startY )
	{
		super( ApoDefenceConstants.ENEMY_SNAKE, iEnemy, enemyWay );
			...
			for ( int i = 0; i < tower.size(); i++ )
			{
				if ( tower.get( i ).getRec().intersects( this.getRangeRec() ) )
				{
					...
					if ( this.isIn( enemyX, enemyY, towerX, towerY ) )
					{
						...
						this.getShoot().add( new ApoDefenceShootEnemy( this, this.getX() - 3, this.getY() - 3, 6, 6, speedX, speedY, this.getDamage(), Color.GREEN ) );
						break;
					}
				}
			}
